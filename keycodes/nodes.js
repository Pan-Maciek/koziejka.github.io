...
Node.prototype.createBindList = function (interpreter, filter = () => true) {
    ...
    const set = (newList, filter) => {
        ...
        let filteredList = newList.filter(filter)
...
Node.prototype.select = function (filter, show = "ELEMENT") {
    if (typeof filter == "string") {
        ...
        let list = {}
        if (show.toLowerCase() == "data") {
            if (this.hasBindList) {
                let last = 0
                for (let i = 0; i < this.bindList.length; i++) {
                    if (filter(this.bindList[i]) == true) {
                        list[last] = {
                            value: this.childNodes[i]
                        }
                        last++
                    }
                }
                list.length = { value: last }
            }
        } else {
